...
int timediff1(struct time arr, struct time dep)
{   
    if(arr.hh<dep.hh){
        arr.hh+=24;
    }
    ...
    int result=(arr.hh-dep.hh)*60+(arr.mm-dep.mm);
    return result;
}

// int timediff3(int day_2,int day_1,struct time arr, struct time dep){
//     if(day_1 == day_2){
//         return timediff1(arr,dep);
//     }
//     else if((day_2 - day_1) < 0){
//         day_2+=7;
//     }
//     int date_diff = day_2 - day_1-1;
//     return date_diff*24*60 + timediff1(arr,dep);

// }


int datediff(struct date d1,struct date d2){
    if(datetoday(d1.day,d1.month,d1.year)< datetoday(d2.day,d2.month,d2.year))
        return 1;
    else
        return 0;
}
